...
void MainMenu::display(sf::RenderWindow &win, sf::Shader *shader)
{
    ...
    win.draw(_gameObject[_save.level1 ? 5 : 1]->getSprite());
    win.draw(_gameObject[_save.level2 ? 6 : 2]->getSprite());
    win.draw(_gameObject[_save.level3 ? 7 : 3]->getSprite());
    if (!_anim) {
        win.draw(_gameObject[4]->getSprite());
